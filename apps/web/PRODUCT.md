...
The product is presented to users as **Zephyr**; **AgentOS** is the underlying architecture/codebase name and should not appear in user-facing copy (confirmed against the shipped UI: `index.html` title/meta, `AppShell.tsx`, and `Nav.tsx` all brand as "Zephyr"). The user wants a distinctive, carefully designed interface rather than a generic AI dashboard. Visual direction has not yet been selected.
...
- The composer starts a task with one call, `POST /api/runs { kind: "agent", input: { goal } }` (`api.startAgentTask`); there is no separate conversation/graph-synthesis API in the current backend. The graph editor (`kind: "graph"`) remains a separate, optional workflow-authoring surface, not part of the submit-to-run path.
...
- Pause/resume is implemented: `POST /api/runs/:id/pause` and `/resume`, with a cooperative pause that takes effect at the next step boundary and a `paused` run status. Live controls may claim to pause a backend run.
...
- Name: Zephyr (user-facing), not the FlowAI label in the reference images. AgentOS is the architecture/codebase name and stays out of user-facing copy.
...
- Backend-owned contracts for any new harness lifecycle operations beyond the confirmed pause/resume.
